...
if __name__ == '__main__':
    ...
    agents = [Agent(alpha = 0.0001, beta = 0.001, input_dims = input_dims, tau = 0.001,
            batch_size = batch_size, fc1_dims = 400, fc2_dims = 300, n_actions = action_dims) for i in range(n_drones)]

    memory = ReplayBuffer(max_size, input_dims, action_dims)
    ...
    for episode in range(n_episodes):
        ...
        for i in range(n_drones):
            observations[(i*6):(i*6+6)] = [np.random.uniform(X_MIN, X_MAX), np.random.uniform(Y_MIN, Y_MAX), np.random.uniform(Z_MIN, Z_MAX), 0, 0, simulation_index]
        ...
        while len(drones) > 0:
            ...
            # Distance based Observation_Space
            for i in drones:
                action = agents[i].choose_action(observations)
                observation_, reward, done, info = env.step(i, action)
                remember(observations, action, reward, observation_, done)
                if memory.mem_cntr >= batch_size * 10:
                    mem_states, mem_actions, mem_reward, mem_states_, mem_done = \
                        memory.sample_buffer(batch_size, i)
