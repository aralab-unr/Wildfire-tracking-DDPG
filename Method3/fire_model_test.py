...
class FireEnvironment(gym.Env):
    ...
    class Agent(object):
        def __init__(self, pos, fov, fsen, tstep):
            self.pos = pos
            self.fov = fov
            self.fsen = fsen
            self.tstep = tstep
    ...
    def reset(self, observations):
        ...
        for i in range(self.num_agents):
            agents[i] = self.Agent(observations[(i*6):(i*6+3)], self.theta, observations[(i*6+3):(i*6+5)], observations[(i*6+5)])
        self.agents = agents

        self.env_fallout = np.zeros(self.num_agents)

    def state(self):
        x = np.empty((self.num_agents * 6))
        for i in range(self.num_agents):
            x[(i*6):(i*6+3)] = self.agents[i].pos
            x[(i*6+3):(i*6+5)] = self.agents[i].fsen
            x[(i*6+5)] = self.agents[i].tstep
        return x

    def step(self, drone, action):
        done = self.steps >= self.max_steps
        success_text = {'success!': done}
    ...
    def Render(self):
        ...
        for t in self.fire_info:
            temp_img = cv2.circle(temp_img, (t.x, t.y), 1, (255,255,255), -1)
    ...
    def view_masks(self):
        view_masks = {}
        for i in range(self.num_agents):
            view_masks[i] = self.view_ind_mask(i)
        return view_masks
